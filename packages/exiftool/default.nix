...
let
  ...
  # /usr/lib/libSystem.B.dylib. This works identically on Linux (pkgsStatic is
  # the fully-static set there).
  ...
  IOCompress = perlPackages.IOCompress.overrideAttrs (old: {
    propagatedBuildInputs =
      (lib.filter (
        d: !(lib.elem (d.pname or "") [ "Compress-Raw-Zlib" "Compress-Raw-Bzip2" ])
